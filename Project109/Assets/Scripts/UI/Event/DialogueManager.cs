...
public class DialogueManager : MonoBehaviour
{
    ...
    // 1. 재화 (골드) 제어 API
    public int GetGold() => RunManager.instance.player.playerStat.InGameCurrencyGold;

    public void ConsumeGold(int amount)
    {
        RunManager.instance.player.playerStat.InGameCurrencyGold = Mathf.Max(0, GetGold() - amount);
    }

    public void AddGold(int amount)
    {
        RunManager.instance.player.playerStat.InGameCurrencyGold += amount;
    }

    // 2. 카드 제어 API
    public int GetCardCount()
    {
        return RunManager.instance.player.deck.GetCards().Count;
    }

    public string GetCardNameAt(int index)
    {
        var cards = RunManager.instance.player.deck.GetCards();
    ...
    public void RemoveCardAt(int index)
    {
        ...
        if (index >= 0 && index < cards.Count)
        {
            RunManager.instance.player.deck.RemoveCard(cards[index].runtimeID);
    ...
    {
        return RunManager.instance.player.relicManager.GetRelics().Count;
    }

    public string GetRelicNameAt(int index)
    {
        var relics = RunManager.instance.player.relicManager.GetRelics();
    ...
    public void RemoveRelicAt(int index)
    {
        ...
        if (index >= 0 && index < relics.Count)
        {
            RunManager.instance.player.RemoveRelic(relics[index].Data.relicName);
    ...
    {
        RunManager.instance.player.RemoveRelic(relicName);
    ...
    {
        var character = RunManager.instance.player.character;
    ...
    {
        var stat = RunManager.instance.player.character.curCharacterStat;
        stat.maxHealth = Mathf.Max(1f, stat.maxHealth + amount);
        if (RunManager.instance.player.character.curHealth > stat.maxHealth)
        {
            RunManager.instance.player.character.curHealth = stat.maxHealth;
        }
    }
