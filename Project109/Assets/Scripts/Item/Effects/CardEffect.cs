...
public enum ConditionType   //카드 조건 타입
{
    None,
    Target_Health_Percent,      //대상 체력 퍼센트
    Buff_Count,                 //버프 갯수
    Frenzy_Count,               //광분 갯수
    Frenzy_Per,                 //광분 버프 당
    IsPoisoned,                 //중독 상태인지
    IsDeadlyPoisoned,           //맹독 상태인지
    IsBleeding,                 //출혈 상태인지
    IsDebilitated,              //쇠약 상태인지
    IsWeakened,                 //약화 상태인지
}
...
{
    public abstract void ApplyEffect(GameObject player, GameObject target, CardStat cardStat);
}